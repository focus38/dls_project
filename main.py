...
# Функция обработки изображения (заглушка)
async def process_image(image_uuid: str, input_path: str):
    try:
        # Здесь должна быть ваша логика обработки изображения
        # Для примера просто ждем 10 секунд
        await asyncio.sleep(10)
        
        # Сохраняем "обработанное" изображение (в реальности то же самое)
...
# Эндпоинт для проверки статуса изображения
...
# Эндпоинт для получения обработанного изображения
...
# Эндпоинт для отмены обработки (опционально)
@app.delete("/cancel/{image_uuid}")
async def cancel_processing(image_uuid: str):
    if image_uuid in processing_tasks:
        processing_tasks[image_uuid].cancel()
        processing_tasks.pop(image_uuid, None)
        return {"status": "cancelled"}
    else:
        raise HTTPException(status_code=404, detail="Image not in processing")